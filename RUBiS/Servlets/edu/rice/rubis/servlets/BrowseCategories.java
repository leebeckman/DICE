...
public class BrowseCategories extends RubisHttpServlet {
	...
	private boolean categoryList(int regionId, int userId,
			PreparedStatement stmt, Connection conn, ServletPrinter sp) {
		String categoryName;
		int categoryId;
		ResultSet rs = null;
		...
		try {
			...
			do {
				...
				if (regionId != -1) {
					...
					if (userId != -1)
						sp.printCategoryToSellItem(categoryName, categoryId,
								userId);
					else
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ((username != null && username != "")
			...
			if (userId == -1) {
				sp.printHTML("<p> You don't have an account on RUBiS!<br>You have to register first.</p>");
				sp.printHTMLfooter();
				closeConnection(stmt, conn);
				return;
			}
		...
		if ((value != null) && (!value.equals(""))) {
			...
			try {
				stmt = conn
						.prepareStatement("SELECT id FROM regions WHERE name=?");
				stmt.setString(1, value);
				ResultSet rs = stmt.executeQuery();
				if (!rs.first()) {
					sp.printHTML("<p> Region " + value
				...
				sp.printHTML("<p>Failed to execute Query for region: " + e + "</p>");
