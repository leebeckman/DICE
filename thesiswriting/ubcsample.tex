...
\paragraph{Summary}
The experimental goals were met in that we were able to identify large sections of a critical page which could be pregenerated. As 
Identify the sections where are actually precomputable\\ by going through results, precompute those and mix them with \\the dynamic content.
...
\paragraph{Summary}
Identified a critical page which could be entirely cached.
Identify the cache keys and invalidating data sources,\\ implement a cache to return the entire page given a key.
...
\paragraph{Summary}
Identified portions of a page which could be cached.
Investigate the result trace to determine how feasible\\ the cache is given the complexity of jGossip's architecture and \\dependence on autogenerated code. Potentially create a cache\\ for a small portion of the page, and explore the results to see\\ if more of the response page can be cached.
...
\paragraph{Experimental Goals}
The goal here was for the analysis to identify common computations that can be deferred. The output should have it clear at which points in the application logic the computations occur for the purpose of converting them to asynchronous versions.
...
\paragraph{Summary}
Identified a database update and subsequent update\\ verification which could be deferred for a commonly\\ performed request.
modify the applications to perform the identified\\ computations asynchronously and assess the correct \\operation of the site.
...
\paragraph{Summary}
Identified a database update and logging action which\\ could be deferred for an infrequently performed request.
modify the applications to perform the identified\\ computations asynchronously and assess the correct \\operation of the site.
...
\paragraph{Summary}
Identified all in-memory persistent state in the application.
If necessary, modify the application to store this state in\\ a database. Use the knowledge of which application\\ components communicate persistent data to assess the \\viability of replicating the computation those components do.
...
\paragraph{Summary}
 Identified persistent session data and the \\sequence of components responsible for communicating it to the user.\\ Determined that this data was only shared with a single user.
 Potentially move the persistent data out of the server-side\\ and into code the client runs. Use the trace to determine what\\ logic the client needs to properly work with the data.
...
\paragraph{Summary}
Identified almost 60\% of edges as carrying unnecessary\\ data in a trace of a commonly accessed page.
Use these results to obtain better communication \\cost bounds in an application partitioning algorithm.\\ Use the results to identify potential application modifications to\\reduce wasteful inter-component communication.
