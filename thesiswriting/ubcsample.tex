...
\chapter{Introduction - BOOK DENTIST APPOINTMENT} % DIFFICULT
...
- different approaches to the problem (this is probably the hardest thing I'll have to deal with)
	...
	- one point will be that my analysis is exploratory in nature. At this point I'm not out to really beat any related attempts in quantifiable ways, but rather to explore this area in a newish application.
...
Chapter: Introduction (really want to draw people in here, high level what is going on)
	
	Sectionless: Statement of problem
	...
	Section: Motivating Example DEF WANT TO TALK ABOUT PARTITIONING, PRETTY CRAAAAZY MIGRATION (ALSO USE PPT SLIDE AS INSPIRATION)
	
	Consider a simple online shopping site, the basic architecture of which is presented in Figure Blah. 
...
%	- make the case for the data being key for many instances, and so it seems natural to see what we can do with traces of where all the data goes.	
	...
	Many research efforts and general techniques exist to address this need, some of which focus on very specific cases and others which go for more general, broad-based analyses. These are discussed in the following related work chapter in order to motivate and position our own work, but it is useful to immediately identify several key elements of our project.% concerning its origins. 
...
Chapter: Related Work
	Here is where we delve deeper into related work, to really localize what I'm doing
	Section: Migration support, something to say that migration is hard
	Section: Taint Tracking/DIFT/Tomography (with a focus on visualization)/DAP
	Section: AOP/Instrumentation/Profiling in General
	Section: Dataflow graph analysis
	Section: Application Partitioning - very important motivator
		-originally developed to be complementary to a partitioning analysis
		-"which seeks to split applications into pieces and then optimally distribute those pieces at different locations while considering computation requirements at each location and communication costs between them."

start off by immediately introducing the problem at a high level. Talk about migrating applications, understanding them, being faced with optimization and problem avoidance (can mine refs from Fluxo).
