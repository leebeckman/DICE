...
\begin{itemize}
\item location Tag \\ <location callerClass, callerMethod, calledClass, calledMethod, requestCounter, requestURI, requestRemoteAddr, callerContextCounter, calledContextCounter /> This tag appears in every log entry, and contains all information needed to determine where the event occured. The attributes of this tag are as follows: \\
...
[called/called]ContextCounter - these attributes supplement the [caller/called][Class/Method] by identifying unique instances of method calls. Every

\item Xobject Tag \\ <Xobject taintID, type, objectID, value, taintRecord /> Various 'object' tags (sourceObject, taintedObject, etc) appear in log records to describe important Objects in taint flow events.

\item field Tag \\ <field targetClass, targetField> These tags are used in field set/get records to identify the field in question.

\item taintRecord Attribute \\ TARGETCOLUMN: column CATALOG: catalog TABLE: table COLUMN: column || URI: uri PARAM: param

\item Propagation Log \\ <taintlog type="PROPAGATION"><location tag><sourceObject><destObject></taintlog> Used to indicate that one tainted Object was derived from another.

\item Fuzzy Propagation Log \\ Used to indicate that an Object was marked as tainted heuristically by fuzzy propagation, as described earlier

\item Modification Log \\ t

\item Composition Log \\ t

\item Association Log \\ t

\item Calling Log \\ t

\item Returning Log \\ t

\item Returning Args Log \\ t

\item Returning Input Log \\ t

\item Output Log \\ t

\item Non-Taint Output Log \\ t

\item Field Set Log \\ t

\item Field Get Log \\ t
...
\paragraph{The Visualization System} relies primarily on the JUNG framework to provide a more user-friendly means of working with the taint tracking data. To use the tool, a log file is loaded, which is then used to generate JUNG graphs 

% Probably want to talk about some graph preprocessing that we do

\paragraph{Automated Analyses}
