...
rubis post computation DIAGRAM COMPLETE
rubis state finding DIAGRAM COMPLETE
rubis user state finding CURRENTLY NO USER STATE IN APPLICATION <- SKIP
rubis useless comm APPLICATION TOO SIMPLE TOO EXHIBIT MUCH OF THIS(MAY NOT BE ANY) <- SKIP

jgossip precomp <------ DONE BEFORE, SHOULDN'T BE TOO BAD
jgossip caching <------ INPUT COULD BE A FORUM ID, TO DISPLAY A THREAD
jgossip post computation <------ FOUND THIS IN FORM OF CREATING A POST
jgossip user state finding <------ THIS WORKED WITH SINGLE USER, NEED TO SPOOF MULTIPLE USERS
jgossip useless comm <----- CAN JUST RUN THIS ON ANYTHING


implementation modifications (referring to diagrams, maybe trimming it down a bit)
implementation analysis algorithm descriptions
maybe some future work stuff
...
The problem which motivates this work is a broad one, and thus is one which has been the subject of a wide variety of attempts to solve it, or at least offer some assistance. We refer to the task of automated application understanding and optimization, in our case specifically dealing with web applications and in the context of such an application being faced with changes to its operating environment, or a need thereof. It is frequently the case that over the lifetime of these applications new demands will be placed on them which encourage change and adaptation, such as increased load from a growing user base or availability of new technologies. This is termed 'migration', which could imply changes ranging from a significant moving of the application to new hardware with large architectural changes to merely modifying some small part of the system to make better use of the resources available to it. The main point is that such tasks are often very difficult to perform manually, especially when done by non-expert developers who must first seek an understanding of the complex applications in question. Gaining such an understanding can be time-consuming, and even then the changes made to the application are prone to errors as developers may miss consequences of their changes. As such, automated analysis tools have been extensively sought after. The goal of such tools is to reduce the need for human experts of existing applications - to support non-experts in gaining an understanding of a system and even suggesting and/or making beneficial changes to it with little human intervention required. %Presented in this thesis is one such system, and we will begin with a motivating example to illustrate some of the problems we are interested in.
	
\section{Origins of Research}
	
Many research efforts and general techniques exist to address the need for this kind of automated analysis, some of which focus on very specific cases and others which go for more general, broad-based analyses. These are discussed at length in the related work chapter which follows in order to motivate and position our own work, but it is useful to immediately identify several key elements of our project concerning its origins. 

Firstly, the TaTAMi project is a system which uses a technique variably referred to as dynamic information flow tracking (DIFT), taint tracking, and dataflow tomography to support a variety of analyses for Java web applications. Essentially, DIFT is a technique where data is tainted (or tagged) so that its progress can be tracked throughout a system, allowing one to see what parts of the system certain data interacts with and how new data is derived from existing data. The method has been used almost exclusively in the security and privacy domains, but we realized that for many of the analyses that we wanted to perform, knowledge of the flow of an application's data throughout its components was key. We anticipated that a certain variant of taint tracking, dataflow tomography, which seeks to discover software behavior from data flow traces, could extract data which could support varied analysis cases. 

The analyses in question, for this project, have been largely inspired by works focusing on various forms of dataflow optimization. Such includes application partitioning, which seeks to split programs into functional pieces and optimally distribute them, and works such as the Fluxo system [ref]. Fluxo provided a series of automatic analyses which could be performed over dataflow graph representations of programs, including discovering potential caches, finding opportunities to delay computation to lower latency, and differentiating between stateful and stateless components. Many of the analyses developed for TaTAMi were derived from those presented in the Fluxo system, though TaTAMi operates under considerably fewer constraints, as will be discussed later.	
...
To further motivate the TaTAMi system and especially illustrate the natural fit of dataflow tracking to the problem at hand, consider the following example. Presented is a simple online shopping site, the basic architecture of which is given in Figure~\ref{fig:cap1}. The system has back-end data stores for shop inventory and user account info and the logic to access them. For interacting with the site, there are two front-end components: the browsing component which accepts requests from the user to fetch groups of shop items for display, formatted by the page render component; and the purchasing component, which allows users to maintain a 'shopping cart' list of items they wish to buy (kept in a persistent 'Order State', which could be a session attribute) and ultimately bill an order to their account. Now, imagine that this system needs to be upgraded to meet increased user demand. The developers responsible for this will need to have a good understanding of the site's operation in order to make informed modifications which do not cause more problems than they solve. As stated, we believe that capturing the dataflow of this system will naturally support developers in this effort, and as such Figure~\ref{fig:cap2} presents a possible dataflow for the system that our tool would obtain. The points to note here are the flow of shop item data from the Inventory database through the browsing system to the user, and the contribution of data from both back-end stores and user input into the 'Order State' shopping cart data. 

Looking first to the flow of data from the Inventory data to the user, additionally consider Figure~\ref{fig:cap3} and Figure~\ref{fig:cap4}, which outline two important dataflow cases. The first shows how the dataflow could indicate that the data used to generate responses to user browse requests could be entirely from predictable sources. Say the browsing involves simply displaying all of the item names, and the set of available items rarely changes, then the output browsing page would remain the same for long periods. The application could potentially be optimized by pregenerating the browsing page and serving it directly to the user, rather than going to the database. Figure~\ref{fig:cap4} shows an alternate dataflow where user input is tracked and determined to be also responsible for the browsing page. Such could arise, say, if the user supplies some filters to search for items. The input may be very unpredictable, as is indicated here, such as for a free text search, and thus the output browsing page may also be unpredictable and thus a poor candidate for any kind of precomputation. On the other hand, if the user input occurs over some reasonably predictable range, such as if the items were being filtered over a set range of categories, the various output browsing pages from this input may be good candidates for cachable data. An automated analysis could point this out, and thus a developer is aided applying this performance improving optimization, being given such useful insights as which data sources to use for cache keys (user input parameters) and which to check for cache invalidation (the Inventory database).

Looking now to the flow of data around the Order State persistent store, another use of dataflow tracking emerges. First and quite simply, dataflow can potentially pinpoint with ease and accuracy various points in a program where persistent data is stored. By following data into the Order State area, and observing that it persists across multiple requests to the application, an automated analysis can determine that this data is used to communicate data beyond the scope of a single request. Identifying such data is useful as it marks the distinction between stateful and stateless components of a system. Stateful components may necessitate careful handling in certain environments, such as when computation involving those components is to be replicated, or when a new environment may not allow a component to store its state reliably. Again, knowledge of an application's dataflow has the potential to support understanding. Taking this analysis further, Figure~\ref{fig:cap5} alludes to the possibility of a dataflow tracking which might reveal how certain data items only flow to and from certain users. For data constituting persistent state, this might be personal data which is not used to communicate between multiple users, such as one's own shopping cart. Once such state is identified by an analysis, one might be able to make decisions like moving the state from the server into the client, potentially making the application more robust.

If these examples seem scattered at this point, bear in mind that they have largely been inspired by the literature and that the point of our work is to explore a variety of analyses. Continuing, another interesting kind of rearchitecting that might be performed on our web store is to partition it. Figure~\ref{fig:cap7} demonstrates a such a scheme, where the two front end components are moved into separate, independent  deployments, communicating with the backend over the network when database data is needed. Such a change could increase the user serving capacity of the application by providing more processing power to the front-end systems to render pages. Application partitioning algorithms generally seek to identify ways to split of application components in a way which makes efficient use of resources while carefully managing communication overhead between partitions. Generally this involves profiling the application to determine component execution and intercomponent communication costs. Obtaining a more complete view of the dataflow can provide deeper knowledge into, say, this communication. Not only can one know when components share data, but can additionally find out where that data is eventually used. In some cases, the data may never be used at all, and thus the dataflow tracking reveals aspects of the application which could be useful when partitioning it.
...
The main contributions of this work are as follows: the construction of a dataflow tomography tool for Java web applications, able to track tainted Strings and numerical values throughout a single program and report wherever they flow; the construction of an analysis tool to both visualize tomographic data and run analyses over it; the identification and implementation of several analyses designed to operate over tomographic data; and the demonstration that these analyses can be applied to real applications successfully.

\section{Thesis Organization}
	
The remainder of this thesis is organized thusly: Chapter 2 discusses the body of related work to cover attempts to address related problems and also to show how some of the techniques we use have not been sufficiently explored in this space. It also covers some relevant background material for technologies used in this project. Chapter 3 covers the implementation of the dataflow tomography tool and the analysis tool, breaking down their key components and explaining important design decisions. Chapter 3 presents an evaluation of the tools, demonstrating the output of the tomography tool and the results of the automated analyses. Chapter 4 finishes with a discussion of the results from the evaluation, possible future work, and overall conclusions.
	
\chapter{Related Work}

\section{Dynamic Information Flow Tracking}
	
	The main stream of research contributing to this project is that of dynamic information flow tracking (DIFT), a technique which may alternately be referred to as tain tracking or dataflow tomography. Introduced by Suh et al. in \cite{Suh2004}, DIFT refers to tagging (tainting) data used by an application at runtime so that its flow through the application can be tracked. Often one works in terms of various interesting sources - places in an application where input is received, such as by reading from a database or receiving input from a user - where data coming from a source in tainted. From the sources, DIFT systems employ some set of propagation rules which define how tainted data is spread through the system. As a simple case, when a variable is produced by copying a tainted input value, the taint tag should be copied from the input value to the new variable. Finally, DIFT systems will establish a set of monitoring sink points in the application to check and report if any tainted data flows through there. These can include entities from the arguments to a certain function or even specific memory locations, depending on the needs of the tool. Many DIFT systems do taint tracking at a fairly low level, down to the level of individual bytes of an application's memory in order to support such security analyses as memory corruption detection. Taint tracking has been used almost exclusively in the domain of security/privacy, and such systems include \cite{Suh2004} \cite{Newsome2005}, where it is to detect if bytes from untrusted input sources like user input ever end up tainting such sinks as jump target addresses or executable instructions; \cite{Hudgins2004}, where it is used trace sensitive data like passwords throughout a system to see where and for how long sensitive data is potentially accessible; \cite{Dalton2007}, where the focus shifts to higher-level SQL injection and cross-site scripting attacks, checking if user input taints such sinks as database queries; and \cite{Al-Saleh2010} \cite{Al-Saleh2010a}, which identifies malware and network protocol analysis as uses. \cite{Al-Saleh2010} is also interesting in that it focuses on a taint propagation policy known as control flow dependence, where values are tainted if they are assigned values in the scope of a control flow statement (such as an if branch) predicated on tainted data. Such propagation is difficult as it often leads to an explosion of taint in the system, and must be very carefully managed, and our own system does not use it as an intentional design decision. \cite{Gupta2008} additionally identifies data lineage tracing as a use, which focuses on determining the inputs responsible for outputs in various scientific computations, which is similar to some of the analyses we do. \cite{Dalton2010}, \cite{Zavou2011}, and \cite{Kangkook2012} present more security-focused systems, as indeed the majority of taint tracking tools are.
	\cite{Mysore2008} presents something different, and was one of the key pieces in developing our own system. Unlike the previous systems, which are security focused and generally only place sinks at critical points where tainted data is not supposed to reach, dataflow tomogrpahy as introduced in this work does a heavier form of tracking. In this work, tainted data is reported as it flows through every function call (or even instruction) with the ultimate goal of providing a complete end-to-end picture of how data flows through a system. The goal of this system is to help users understand a target application, as is ours, and the works shares our attention to visualization, noting the difficulty of displaying what our often very large flow graphs, as well as our qualitative evaluation strategy. They make the point that while most uses of DIFT are focused on policy enforcement, as with security and privacy, their own work is one which focuses on using the analysis for discovery. They do not, however, give any concrete uses for their analysis beyond general understanding, and as such we feel our own work fills some of this void.
	\cite{Haldar2005} and \cite{Chin2009} present Java taint tracking systems, like ours, which modify the Java runtime to track taint in Strings. By contrast, our system uses AspectJ to obviate the need for such modifications, and we additionally have limited support for tracking taint in numerical values, as described in the Implementation chapter. These systems are, like other efforts, focused on preventing untrusted data from propagating to secure locations.
	
\section{Data Update Propagation}
	Data Update Propagation (DUP) is a line of research presented in \cite{Challenger1999} \cite{Degenaro2000} \cite{Challenger2001} \cite{Challenger2004} and \cite{Challenger2005} by Arun Iyengar et al. Essentially, DUP is a scheme designed to help consistently cache dynamic data. It does this by maintaining a depedence graph between underlying data sources like database tables and cached objects, through any intermediate objects along the way. When underlying data is modified, the dependence graph is traversed to discover which cached objects need to be invalidated. In the first paper, the chief limitation of this strategy is the requirement that the application (and thus the developer) maintain the dependence graph itself through the use of a provided API. DUP is similar to the caching analysis performed in this thesis, where data flow graphs are searched to discover possible caches and all relevant inputs to them. In our case, however, the graphs are created automatically, without the requirement on the developer to manually specify them. \cite{Degerano2000} does automatically generate dependence graphs, but only because the system cache at the level of immediate query results and the depedence graphs come from query analysis. \cite{Challenger2001} uses the same manual DUP strategy, but introduces such useful concepts as web page fragmenting, where dynamic fragments of a page are identified and able to be tied to underlying data, as well as page pregeneration. This last is much like another of the analyses we perform, known as precomputation, which seeks to find computation results, and even parts of output webpages which can simply be pregenerated due to being based on infrequently changing data.

\section{Automated Analysis and Decision Support}
	
	We present here an assortment of works which in some way seek to automatically aide developers in similar manners to our own work, and which use similar ideas to us. Fluxo \cite{Kiciman2010} is a system from which we drew significant inspiration. In Fluxo, developers write applications in a restricted, higher level language which can then be compiled into a dataflow graph representation of the program. This representation is similar to what our own system obtains by monitoring dataflow in an application, though ours operates on unmodified, existing programs and thus the graphs tend to be less restricted and more difficult to work with. The paper supplies a series of tranformations which can be applied to these graph representations to optimize the programs, with the goal of reducing end-to-end latency and addressing what they identify as a need to save developers from having to manually determine the best use of their infrastructure. These analyses include discovering caches, identifying precomputable data, finding computations which can be delayed until later, and separating stateful and stateless components. In Fluxo these optimizations can be applied automatically when found, due to the use of the restricted programming model, whereas in our work this is not yet the case. However, we do support analyses to at least direct developers to where they can apply similar optimizations, and do so for legacy software systems.
	The Fluxo paper is preceded by \cite{Rasmussen2009}, which gives greater attention to the problem of discovering caches in a dataflow graph. Most imporantly, this paper identifies some key points to consider when searching for caches which were encountered in our own work. These include attempting to choose caches which will actualy provide worthwhile execution savings and being sure that adding a cache does violate the semantics of the application, such as by skipping executions which have important side effects besides producing the cached data.
...
\subsection{RUBiS Input Source Description File [may need to rename this]}
DEAL WITH ALL OF THESE UNDERSCORES
<root>
    <databasesourceinfo catalog="rubis" table="categories" column="id" variability="STABLE" inttracking="true" />
    <databasesourceinfo catalog="rubis" table="categories" column="name" variability="STABLE" />
    <databasesourceinfo catalog="rubis" table="regions" column="id" variability="STABLE" inttracking="true" />
    <databasesourceinfo catalog="rubis" table="regions" column="name" variability="STABLE" />
    <databasesourceinfo catalog="rubis" table="users" column="id" variability="STABLE" inttracking="true" />
    
    <requestsourceinfo uri="/rubisUNDERSCOREservlets/edu.rice.rubis.servlets.SubmitChat" parameter="chatMessage" variability="RANDOM" />

    <requestsourceinfo uri="/rubisUNDERSCOREservlets/edu.rice.rubis.servlets.BrowseCategories" parameter="region" variability="PREDICTABLE" />
    <requestsourceinfo uri="/rubisUNDERSCOREservlets/edu.rice.rubis.servlets.BrowseCategories" parameter="nickname" variability="PREDICTABLE" />
    <requestsourceinfo uri="/rubisUNDERSCOREservlets/edu.rice.rubis.servlets.BrowseCategories" parameter="password" variability="PREDICTABLE" />
</root>

\subsection{jGossip Input Source Description File [rename?]}
<root>
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREforum" column="forumtitle" variability="STABLE" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREforum" column="forumdesc" variability="STABLE" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREforum" column="forumid" variability="STABLE" inttracking="true" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREforum" column="locked" variability="STABLE" inttracking="false" />

    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREgroup" column="groupUNDERSCOREname" variability="STABLE" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREgroup" column="groupid" variability="STABLE" inttracking="true" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREgroup" column="groupUNDERSCOREsort" variability="STABLE" inttracking="false" />

    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREmessage" column="cont" variability="RANDOM" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREmessage" column="mUNDERSCOREfrom" variability="RANDOM" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREmessage" column="id" variability="RANDOM" inttracking="true" />

    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREthread" column="tUNDERSCOREstamp" variability="PREDICTABLE" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREthread" column="tid" variability="PREDICTABLE" inttracking="true" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREthread" column="sortby" variability="PREDICTABLE" inttracking="false" />

    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREwhois" column="id" variability="PREDICTABLE" inttracking="true" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREwhois" column="ip" variability="RANDOM" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREwhois" column="sessionid" variability="RANDOM" inttracking="false" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREwhois" column="userUNDERSCOREname" variability="PREDICTABLE" inttracking="false" />

    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREskinUNDERSCOREparams" column="paramUNDERSCOREname" variability="STABLE" />
    <databasesourceinfo catalog="forumUNDERSCOREdb" table="jrfUNDERSCOREskinUNDERSCOREparams" column="paramUNDERSCOREvalue" variability="STABLE" />

    <requestsourceinfo uri="/jgossip/jgossip/DeleteForum.do" parameter="fid" variability="RANDOM" inttracking="true" />
    <!--
    <requestsourceinfo uri="/rubisUNDERSCOREservlets/edu.rice.rubis.servlets.BrowseCategories" parameter="region" variability="PREDICTABLE" />
    <requestsourceinfo uri="/rubisUNDERSCOREservlets/edu.rice.rubis.servlets.BrowseCategories" parameter="nickname" variability="PREDICTABLE" />
    <requestsourceinfo uri="/rubisUNDERSCOREservlets/edu.rice.rubis.servlets.BrowseCategories" parameter="password" variability="PREDICTABLE" />-->
...
\begin{figure}[ht]
  \begin{center}
    ...
    \caption[RUBiS Browse Categories With Chat Trace]{\label{fig:browsecategorieschat} RUBiS Browse Categories With Chat Trace.}
  \end{center}
\end{figure}
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/jgossip_MainPage_Precomp.eps}
    \caption[jGossip Main Page Precomputation Analysis Results]{\label{fig:mainpageprecomp} jGossip Main Page Precomputation Analysis Results.}
  \end{center}
\end{figure}
...
NOTES: NEED TO REDEFINE FORMAT OF DATABASE NODES
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/rubis_BrowseRegionCategories_Numbered.eps}
    \caption[RUBiS Browse Categories By Region Trace]{\label{fig:browseregioncategories} RUBiS Browse Categories By Region Trace.}
  \end{center}
\end{figure}

\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/rubis_BrowseRegionCategories_Caching.eps}
    \caption[RUBiS Browse Categories By Region Caching Results]{\label{fig:browseregioncategoriescaching} RUBiS Browse Categories By Region Caching Results.}
  \end{center}
\end{figure}
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/jgossip_ViewForums_Caching.eps}
    \caption[jGossip View Forums Caching Results]{\label{fig:viewforumscaching} jGossip View Forums Caching Results.}
  \end{center}
\end{figure}
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/rubis_SellItem_Numbered.eps}
    \caption[RUBiS Sell Item Trace]{\label{fig:sellitemtrace} RUBiS Sell Item Trace.}
  \end{center}
\end{figure}

\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/rubis_SellItem_PostComp.eps}
    \caption[RUBiS Sell Item Postcomputation Analysis Results]{\label{fig:sellitempostcomputation} RUBiS Sell Item Postcomputation Analysis Results.}
  \end{center}
\end{figure}
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/jgossip_DeleteForum_PostComp.eps}
    \caption[jGossip Delete Forum Postcomputation Analysis Results]{\label{fig:deleteforumpostcomputation} jGossip Delete Forum Postcomputation Analysis Results.}
  \end{center}
\end{figure}
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/rubis_BrowseItems_State.eps}
    \caption[RUBiS Browse Items State Analysis Results]{\label{fig:browseitemsstate} RUBiS Browse Items State Analysis Results.}
  \end{center}
\end{figure}
...
Notes: In evaluation, note that this is an easy analysis, but is useful for targetting as well as for the related nodes by flow
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/jgossip_BiLogin_UserState.eps}
    \caption[jGossip Login User State Analysis Results]{\label{fig:loginstate} jGossip Login User State Analysis Results.}
  \end{center}
\end{figure}
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{figs/jgossip_ShowForum_WasteComm.eps}
    \caption[jGossip Wasteful Communication Analysis Results]{\label{fig:showforumcomm} jGossip Wasteful Communication Analysis Results.}
  \end{center}
\end{figure}
...
Given the analysis results and with our experiences working with the tools, there are a variety of improvements to be made and interesting future directions to explore. 

System is to be integrated into an application partitioner

Analysis speed (native tracker without AJ)

Testing as an input to other tools, like an application partitioner

Refinement of analysis tool and user testing

Testing with more applications, more extensively. Try to find all that you can about a single application.

Finding more analyses

A more thorough modify and redeploy test, to see how good analysis suggestions are.

More complete tracking

Conflow flow propagation?

Analysis automation (lots of manual intervention required)

Going deeper with some analyses (access path refactoring, useless state)

Automatically acting on some of the analyses, ala fluxo

\section{Final Words}

PROOF OF CONCEPT STATEMENT IN CONCLUSION
Goal not to be exhaustive, but to merely prove the concept. 
